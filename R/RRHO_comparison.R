...
RRHOComparison <- function(list1, list2, list3, 
  ...
  hypermat1 <- matrix(data=NA, 
                      nrow=length(seq(1,nlist1,stepsize)),ncol=length(seq(1,nlist3,stepsize)));
  OR1 <- matrix(data=NA,
                nrow=length(seq(1,nlist1,stepsize)),ncol=length(seq(1,nlist3,stepsize)));
  SE1 <- matrix(data=NA,
                nrow=length(seq(1,nlist1,stepsize)),ncol=length(seq(1,nlist3,stepsize)));
  ...
  hypermat2 <- matrix(data=NA, nrow=length(seq(1,nlist2,stepsize)),
                      ncol=length(seq(1,nlist3,stepsize)));
  OR2 <- matrix(data=NA, nrow=length(seq(1,nlist2,stepsize)),
                ncol=length(seq(1,nlist3,stepsize)));
  SE2 <- matrix(data=NA, nrow=length(seq(1,nlist2,stepsize)),
                ncol=length(seq(1,nlist3,stepsize)));
  countx <- county <- 0;
  ##Loop over the experiments
  ...
  if (plots) {
    ...
    ## Modified from http://www.colbyimaging.com/wiki/statistics/color-bars
      ...
      minhypermat <- hypermat.range[1];
      maxhypermat <- hypermat.range[2];
      pdf(paste(outputdir,
  ...
  return(list(hypermat1=hypermat1,
              hypermat2=hypermat2,
              Pdiff=Pdiff,
              Pdiff.by=Pdiff.by));
}