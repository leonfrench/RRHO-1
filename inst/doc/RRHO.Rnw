...
The test can be performed against a one sided hypothesis (an enrichment hypothesis), or a two sided hypothesis. This is the purpose of this package is based on the work of~\citet{plaisier_rankrank_2010}.
...
As the count of common elements could be driven by chance, the signi???cance of the observed count is computed assuming the hypothesis of completely random list orderings. 
...
The computation of the p-values differ from that described in~\citep{plaisier_rankrank_2010}. 
...
As of version $1.4.0$, a comparison of three lists is possible as described by~\citet{j.l._stein_quantitative_2014}.
